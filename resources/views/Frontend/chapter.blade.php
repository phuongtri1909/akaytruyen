...
@section('content')
    ...
    <style>
        ...
        @media (max-width: 768px) {
            .chapter-content {
                padding: 10px;
                /* Giảm padding để chữ không quá dính vào viền */
                margin: 5px;
                /* Thêm khoảng cách nhỏ để không sát viền màn hình */
                font-size: 16px;
                /* Giảm font-size một chút để đọc dễ hơn */
            }
        }
        ...
        @media (max-width: 480px) {
            .custom-text {
                font-size: 24px;
                /* Giảm nữa trên màn hình nhỏ hơn */
                top: -10px;
    ...
    <div class="chapter-wrapper container my-5">
        ...
        <div class="chapter-nav text-center main">
            ...
            <h1 class="text-center text-success custom-text"><b>Chương {{ $chapter->chapter }}: {{ $chapter->name }}</b>
            </h1>
        </div>

        <!-- <div class="save-chapter-control text-center mt-3">
                            @if (auth()->check() && $isChapterSaved)
                ...
                <div>
                    <style>
                        ...
                        @media (max-width: 480px) {
                            .main {
                                top: -30px;
                                /* Giảm khoảng cách để không bị lệch quá */
                            }
                ...
                <div class="search-wrapper position-relative timkiem">
                    <div class="search-wrapper">
                        <div class="input-group">
                            <input type="text" class="form-control" id="search" placeholder="Tìm Nội dung ...">
            ...
            <div class="chapter-nav d-flex justify-content-between align-items-center mb-4">
                ...
                @auth
                    @if ($story->slug !== 'con-duong-ba-chu-ngoai-truyen')
                        <!-- Kiểm tra slugStory để chặn tải cho truyện này -->
    ...
    <div class="chapter-actions chapter-actions-mobile d-flex align-items-center justify-content-center">
        @if ($chapterBefore)
            <a class="btn btn-success me-2 chapter-prev"
                href="{{ route('chapter', ['slugStory' => $story->slug, 'slugChapter' => $chapterBefore->slug]) }}"
                title=""> <span>Chương </span>trước</a>
        @endif
        <button class="btn btn-success chapter_jump me-2" data-story-id="{{ $story->id }}"
            data-slug-chapter="{{ $slugChapter }}" data-slug-story="{{ $story->slug }}"><span>
        ...
        @if ($chapterAfter)
            <a class="btn btn-success chapter-next"
                href="{{ route('chapter', ['slugStory' => $story->slug, 'slugChapter' => $chapterAfter->slug]) }}"
