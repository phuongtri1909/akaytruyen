...
<div class="comment-list">
        ...
        <div class="avatar-wrapper" style="position: relative; width: 60px; height: 60px; display: inline-block; flex-shrink: 0;">
            <!-- Ảnh Avatar -->
            <img src="{{ $avatar }}"
            ...
            <!-- Ảnh viền nếu có -->
            @if ($border)
                <img src="{{ $border }}" class="rounded-circle"
                    alt="Border {{ $role }}"
                    style="
                        position: absolute;
                        top: 50%;
        ...
        <div class="post-comments p-2 p-md-3">
            <div class="content-post-comments">
                @php
                    $userRole = ($comment->user && $comment->user->roles) ? $comment->user->roles->pluck('name')->first() : null;
                        ...
                        @if ($comment->user)
                            @if ($userRole === 'Admin')
                                <span class="role-badge admin-badge">
                                    @if(auth()->check() && auth()->user()->hasRole('Admin'))
                                        <a href="{{ route('admin.users.edit', $comment->user->id) }}" target="_blank" class="text-decoration-none admin-badge">
                            ...
                            @elseif ($userRole === 'Mod')
                                <span class="role-badge mod-badge">
                                    @if(auth()->check() && auth()->user()->hasRole('Admin'))
                                        <a href="{{ route('admin.users.edit', $comment->user->id) }}" target="_blank" class="text-decoration-none mod-badge">
                            ...
                            @elseif ($userRole === 'vip')
                                <span class="role-badge vip-badge">
                                    @if(auth()->check() && (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Mod')))
                                        <a href="{{ route('admin.users.edit', $comment->user->id) }}" target="_blank" class="text-decoration-none vip-badge">
                            ...
                            @elseif ($userRole === 'Content')
                                <span class="role-badge content-badge">
                                    @if(auth()->check() && (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Mod')))
                                        <a href="{{ route('admin.users.edit', $comment->user->id) }}" target="_blank" class="text-decoration-none content-badge">
                            ...
                            @elseif ($userRole === 'VIP PRO')
                                <span class="role-badge vip-pro-badge">
                                    @if(auth()->check() && (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Mod')))
                                        <a href="{{ route('admin.users.edit', $comment->user->id) }}" target="_blank" class="text-decoration-none vip-pro-badge">
                            ...
                            @elseif ($userRole === 'VIP PRO MAX')
                                <span class="role-badge vip-pro-max-badge">
                                    @if(auth()->check() && (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Mod')))
                                        <a href="{{ route('admin.users.edit', $comment->user->id) }}" target="_blank" class="text-decoration-none vip-pro-max-badge">
                            ...
                            @elseif ($userRole === 'VIP SIÊU VIỆT')
                                <span class="role-badge vip-pro-sv-badge">
                                    @if(auth()->check() && (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Mod')))
                                        <a href="{{ route('admin.users.edit', $comment->user->id) }}" target="_blank" class="text-decoration-none vip-pro-sv-badge">
                            ...
                            @else
                                @if(auth()->check() && (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Mod')))
                                    <a href="{{ route('admin.users.edit', $comment->user->id) }}" target="_blank" class="text-decoration-none text-dark">
                            ...
                            @if($comment->user->email === 'khaicybers@gmail.com')
                                <span class="tooltip-icon">
                                    <img src="https://cdn3.emoji.gg/emojis/64012-management.png" width="30px" style="margin-left:5px; margin-top:-10px;" alt="Hỗ trợ">
                            ...
                            @elseif($comment->user->hasRole('VIP PRO'))
                                <span class="tooltip-icon">
                            ...
                            @elseif($comment->user->hasRole('VIP PRO MAX'))
                                <span class="tooltip-icon">
                            ...
                            @elseif($comment->user->hasRole('VIP SIÊU VIỆT'))
                                <span class="tooltip-icon">
                                    <img src="https://cdn3.emoji.gg/emojis/2336-vipgif.gif" width="30px" style="margin-left:5px;margin-top:-10px;" alt="vipmax">
                                    <img src="https://cdn3.emoji.gg/emojis/53879-bluevip.png" width="30px" style="margin-left:5px;margin-top:-10px;" alt="vipmax">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 flex-sm-nowrap w-100">
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        @if ($comment->level < 1 && auth()->check())
                            <small class="reply-btn text-decoration-underline" style="cursor: pointer;" data-id="{{ $comment->id }}">
                        ...
                        <div class="reaction-wrapper position-relative d-flex align-items-center">
                            ...
                            <!-- Nhóm nút reactions -->
                            <div class="reaction-group d-flex gap-1 p-1 bg-white border rounded shadow-sm" style="
                                position: absolute;
                                bottom: 98%;
                                left: -175px;
                                display: none;
                                z-index: 999;
                            ">
                                <button class="btn btn-sm btn-outline-primary reaction-btn" data-type="like" data-id="{{ $comment->id }}">
                                    <i class="fas fa-thumbs-up"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-secondary reaction-btn" data-type="dislike" data-id="{{ $comment->id }}">
                                    <i class="fas fa-thumbs-down"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-warning reaction-btn" data-type="haha" data-id="{{ $comment->id }}">
                                    <i class="fa-solid fa-face-laugh-squint"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-danger reaction-btn" data-type="tym" data-id="{{ $comment->id }}">
                                    <i class="fa-solid fa-heart"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-danger reaction-btn" data-type="angry" data-id="{{ $comment->id }}">
                                    <i class="fa-solid fa-face-angry"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-warning reaction-btn" data-type="sad" data-id="{{ $comment->id }}">
                            ...
                            @php
                                ...
                                $userReactionType = auth()->check() ? optional($comment->reactions->where('user_id', auth()->id())->first())->type : null;
                            @endphp
...
<script>
    ...
    $('#confirmDelete').click(function() {
        ...
        $.ajax({
            ...
            type: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // Lấy CSRF token từ meta
            },
    ...
    function showToast(message, type = 'info') {
        ...
        $('body').append(`<div class="toast-message" style="position: fixed; bottom: 10px; right: 10px; background: ${bgColor}; color: white; padding: 10px; border-radius: 5px;">${message}</div>`);
...
$(document).ready(function() {
    ...
    $('#confirmDelete').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    showToast(response.message, 'success', true); // Hiển thị thông báo và reload
                    setTimeout(function () {
                    location.reload();
                }, 500);
                } else {
...
$(document).ready(function () {
    ...
    if (response.status === 'success') {
        ...
        $(`.reaction-wrapper[data-id="${commentId}"] .reaction-btn`).removeClass('active');
        btn.addClass('active');
...
<style>
    ...
    *, ::after, ::before {
    ...
    .reaction-like {
    background-color: #0d6efd !important; /* xanh tươi */
    ...
    .reaction-angry {
    background-color: #fd7e14 !important; /* cam */
    color: white !important;
    }

    .reaction-sad {
    background-color: #ffca2a !important; /* vàng đậm */
    color: black !important;
    }
    ...
    .comment-container {
        flex-direction: column; /* Cho bố cục xuống dòng trên màn nhỏ */
        align-items: flex-start;
    }
    ...
    white-space: nowrap;
    padding: 4px 6px;
    min-width: 42px; /* hoặc 40px nếu bạn muốn nhỏ hơn */
